...
$openSpecFile = Join-Path $featureDir 'openspec.json'
...
if (-not $DryRun) {
    ...
    if (-not (Test-Path -PathType Leaf $tasksFile)) {
        $template = Resolve-Template -TemplateName 'tasks-template' -RepoRoot $repoRoot
        if ($template -and (Test-Path $template)) {
    ...
    if (-not (Test-Path -PathType Leaf $researchFile)) {
        Set-Content -Path $researchFile -Encoding utf8 -NoNewline -Value "# Research Notes`n`n## Decisions`n`n## Rationale`n`n## Alternatives Considered`n"
    }

    if (-not (Test-Path -PathType Leaf $openSpecFile)) {
        Set-Content -Path $openSpecFile -Encoding utf8 -NoNewline -Value @"
{
  "$schema": "https://json-schema.org/draft-07/schema",
  "name": "@omni/$Team",
  "version": "1.0.0",
  "description": "OpenSpec contract for $Team feature $branchName",
  "tools": []
}
"@
    }
    ...
    Set-Content -Path $featureStatePath -Encoding utf8 -NoNewline -Value (@{
      feature_directory = "teams/$Team/specs/$branchName"
      team = $Team
    } | ConvertTo-Json)

    # Set the SPECIFY_FEATURE environment variable for the current session
    $env:SPECIFY_FEATURE = $branchName
    $env:SPECIFY_TEAM = $Team
}
