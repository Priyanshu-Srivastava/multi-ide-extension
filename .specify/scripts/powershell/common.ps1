...
function Get-CurrentBranch {
    ...
    # For non-git repos, try to find the latest feature directory across team specs folders
    $teamSpecsDirs = Get-ChildItem -Path (Join-Path $repoRoot "teams") -Directory -ErrorAction SilentlyContinue |
        ForEach-Object { Join-Path $_.FullName "specs" } |
        Where-Object { Test-Path $_ }

    foreach ($specsDir in $teamSpecsDirs) {
        $latestFeature = ""
        $highest = 0
        $latestTimestamp = ""
...
function Test-TeamScopedFeatureDir {
    ...
    $resolvedFeature = Resolve-Path -LiteralPath $FeatureDir -ErrorAction SilentlyContinue
    if (-not $resolvedRepo -or -not $resolvedFeature) {
        return $false
    }

    $repoNorm = $resolvedRepo.Path.Replace('\', '/')
    $featureNorm = $resolvedFeature.Path.Replace('\', '/')

    if (-not $featureNorm.StartsWith($repoNorm + '/')) {
        return $false
    }
