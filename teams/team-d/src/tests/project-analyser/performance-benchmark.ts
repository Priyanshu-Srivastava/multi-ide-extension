...
class BenchmarkReader implements WorkspaceContextReaderPort {
  constructor(
...
class BenchmarkLLM implements LLMPort {
  ...
  async sendPrompt(prompt: string, _context?: LLMContext): Promise<string> {
    ...
    if (prompt.startsWith('Identify and explain the key business and user flows')) {
      return JSON.stringify({
        flows: [
          { name: 'Analyse workspace', owner: 'team-d', steps: ['Gather context', 'Render report'], description: 'Primary reporting flow' },
          { name: 'Review code insights', owner: 'team-d', steps: ['Open code tab', 'Inspect use cases'], description: 'Secondary review flow' },
        ],
    ...
    if (prompt.startsWith('Analyse the TypeScript source code below')) {
      return JSON.stringify({
        ...
        useCases: [
          { name: 'Generate analysis', trigger: 'User starts analysis', steps: ['Gather files', 'Generate report'], outcomes: ['Report visible'] },
          { name: 'Retry analysis', trigger: 'User retries after error', steps: ['Reuse last params', 'Rerun analysis'], outcomes: ['Successful rerun'] },
        ],
        technicalDebt: [],
        diagram: 'flowchart TD\n  Start --> End',
        meta: { confidence: 'high', limitations: [] },
...
function createReader(seed: number): BenchmarkReader {
  const fileSuffix = `${seed}`;
  return new BenchmarkReader(
    {
      'README.md': `# Repo ${fileSuffix}`,
...
async function main(): Promise<void> {
  const llm = new BenchmarkLLM();
  const runDurations: number[] = [];

  for (let index = 0; index < 20; index += 1) {
    const service = new ProjectAnalysisService(createReader(index), llm);
    const start = performance.now();
    await service.generateProjectReport({ depth: 'standard' });
    runDurations.push(performance.now() - start);
  }

  const maxMs = Math.max(...runDurations);
  const avgMs = runDurations.reduce((sum, value) => sum + value, 0) / runDurations.length;
  const withinThreshold = runDurations.filter((value) => value <= 120_000).length;

  console.log(JSON.stringify({
    runs: runDurations.length,
    avgMs,
    maxMs,
    withinThreshold,
    withinThresholdPercent: (withinThreshold / runDurations.length) * 100,
  }, null, 2));
}
